...
// NOTE: IDWriteFontFallback1 is declared by the Windows App SDK but the shipped libdwritecore.so
// shows no evidence of implementing it, so only a forward declaration is kept
// here (it is still named in another interface's signature). See tools/analyze_interfaces.py.
