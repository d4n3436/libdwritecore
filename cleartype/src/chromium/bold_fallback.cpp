...
void MapAtLoad()
{
    ...
    for (int i = 0; i < set->nfont; ++i) {
        ...
        if (get_integer(font, "weight", 0, &fc_weight) != kFcResultMatch ||
            get_integer(font, "slant", 0, &slant) != kFcResultMatch || slant != 0 ||
    ...
    for (const auto& [family, winner] : best) {
        BoldByFamily().emplace(family, winner);
        MapFile(winner.path);
    }
}

// Skia writes -1 for no stroke and useStrokeForFakeBold writes a positive
// width. Another negative is invisible to every test Skia makes of the field
// and is not a value it ever produces.
constexpr float kMarked = -2;
...
bool ClearSyntheticBold(void* rec, const std::vector<uint8_t>& font)
{
    ...
    // A stroke the page asked for itself, which useStrokeForFakeBold widens
    // in place. Marking would throw it away, so the run keeps the synthetic
    // bold Skia gives it.
    if (skia_abi::Read<float>(rec, skia_abi::kRecFrameWidth) >= 0) {
        return false;
    }
    if (RealBoldFor(font).bytes == nullptr) {
        return false;
    }
    ...
    std::memcpy(static_cast<unsigned char*>(rec) + skia_abi::kRecFlags, &cleared,
                sizeof(cleared));
    std::memcpy(static_cast<unsigned char*>(rec) + skia_abi::kRecFrameWidth, &kMarked,
                sizeof(kMarked));
    return true;
}

bool WasMarked(const float frame_width) { return frame_width == kMarked; }
...
Face RealBoldFor(const std::vector<uint8_t>& font)
{
    ...
    // this same face and reached for synthetic bold too.
